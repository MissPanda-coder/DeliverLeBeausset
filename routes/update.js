...
router.put("/:id/shops", (req, res) => {
  ...
  const query =
    "UPDATE shops SET `shopsName` = ?, `shopsstreetnb` = ?, `shopsstreetname` = ?, `shopspostal` = ?, `shopscountry` = ?, `shopsimg` = ?, `shopslogo` = ?, `shopstype_idshopstype` = ? WHERE `idshops` = ?";
  ...
  db.query(
    ...
    (err, results) => {
      if (err) {
        res
          .status(500)
          .json({ error: "Error modifying the shop: " + err.message });
      } else {
...
router.put("/:id/type", (req, res) => {
  ...
  db.query(query, [shopstype, idshopstype], (err, results) => {
    if (err) {
      res
        .status(500)
        .json({ error: "Error modifying the type : " + err.message });
    } else {
...
router.put("/:id/department", (req, res) => {
  ...
  const query =
    "UPDATE department SET `departmentname` = ?  WHERE `iddepartment` = ?";

  db.query(query, [departmentname, iddepartment], (err, results) => {
    if (err) {
      res
        .status(500)
        .json({ error: "Error modifying the department : " + err.message });
    } else {
...
router.put("/:id/units", (req, res) => {
  ...
  db.query(query, [unitsname, idunits], (err, results) => {
    if (err) {
      res
        .status(500)
        .json({ error: "Error modifying the unit: " + err.message });
    } else {
...
router.put("/:id/products", (req, res) => {
  ...
  const query =
    "UPDATE products SET `productsname` = ?, `productsdescription` = ?, `productsprice` = ?, `productsimg` = ?,`productsweight` = ?, `units_idunits` = ?, `department_iddepartment` = ? WHERE `idproducts` = ?";
  ...
  db.query(
    ...
    (err, results) => {
      if (err) {
        res
          .status(500)
          .json({ error: "Error modifying the product : " + err.message });
      } else {
...
router.put("/:id/shd", (req, res) => {
  ...
  const query =
    "UPDATE shops_has_department SET `department_iddepartment` = ?, `products_idproducts` = ?  WHERE `shops_idshops` = ?";

  db.query(
    query,
    [department_iddepartment, products_idproducts, idshops],
    (err, results) => {
      if (err) {
