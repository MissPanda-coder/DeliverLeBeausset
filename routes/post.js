...
router.post("/postshop", (req, res) => {
  ...
  const {
    ...
    shopsimg,
    shopslogo,
    shopstype_idshopstype,
  } = req.body;
  const query =
    "INSERT INTO `shops` (`shopsName`, `shopsstreetnb`, `shopsstreetname`, `shopspostal`, `shopscountry`, `shopsimg`, `shopslogo`, `shopstype_idshopstype`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
  ...
  db.query(
    ...
    (err, results) => {
      if (err) {
        res
          .status(500)
          .json({ error: "Error adding the shop: " + err.message });
      } else {
        res
          .status(201)
          .json({ message: "Shop successfully added", id: results.insertId });
      }
    }
  );
});
...
router.post("/postshopstype", (req, res) => {
  ...
  db.query(query, [shopstype], (err, results) => {
    if (err) {
      res
        .status(500)
        .json({ error: "Error adding the type oh shops : " + err.message });
    } else {
      res
        .status(201)
        .json({
          message: "Type of shops successfully added",
          id: results.insertId,
        });
    }
  });
...
router.post("/postdepartment", (req, res) => {
  ...
  db.query(query, [departmentname], (err, results) => {
    if (err) {
      res
        .status(500)
        .json({ error: "Error adding the department: " + err.message });
    } else {
      res
        .status(201)
        .json({
...
router.post("/postproduct", (req, res) => {
  ...
  const query =
    "INSERT INTO `products` (`productsname`, `productsdescription`, `productsprice`, `productsweight`,`units_idunits`, `department_iddepartment`) VALUES (?, ?, ?, ?, ?, ?)";
  ...
  db.query(
    ...
    (err, results) => {
      if (err) {
        res
          .status(500)
          .json({ error: "Error adding the product: " + err.message });
      } else {
        res
          .status(201)
          .json({
...
router.post("/postunit", (req, res) => {
  ...
  db.query(query, [unitsname], (err, results) => {
    if (err) {
      ...
      res
        .status(201)
        .json({ message: "Unit successfully added", id: results.insertId });
    }
  });
});
...
router.post("/postshops_has_department", (req, res) => {
  const { shops_idshops, department_iddepartment, products_idproducts } =
    req.body;
  const query =
  ...
  db.query(
    query,
    [shops_idshops, department_iddepartment, products_idproducts],
    (err, results) => {
      if (err) {
        ...
        res
          .status(201)
          .json({ message: "Successfully added", id: results.insertId });
      }
    }
  );
});
