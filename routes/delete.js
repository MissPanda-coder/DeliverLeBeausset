...
router.delete('shops/:id', (req, res) => {
    ...
    db.query('DELETE FROM shops WHERE idshops= ?', [id], (err, results) => {
        if (err) {
            res.status(500).json({ error: 'Error deleting the shop : ' + err.message });
        } else {
...
router.delete('shopstype/:id', (req, res) => {
    ...
    db.query('DELETE FROM shopstype WHERE idshopstype= ?', [id], (err, results) => {
        if (err) {
            res.status(500).json({ error: 'Error deleting the shoptype : ' + err.message });
        } else {
...
router.delete('department/:id', (req, res) => {
    ...
    db.query('DELETE FROM department WHERE iddepartment= ?', [id], (err, results) => {
        if (err) {
...
router.delete('products/:id', (req, res) => {
    ...
    db.query('DELETE FROM products WHERE idproducts = ?', [id], (err, results) => {
        if (err) {
